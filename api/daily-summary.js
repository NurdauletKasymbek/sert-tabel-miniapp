...
export default async function handler(req, res) {
  try {
    ...
    const report = buildManagerReport(store, String(req.query?.date || ""));
    const text = [
      "<b>Күндік және айлық табель есебі</b>",
      "",
      report.text,
    ].join("\n");
    ...
    res.status(500).json({ error: `Күндік есеп жіберілмеді: ${error.message}` });
  }
}
